...
DEFINE_bool(server_on,					true,			"Enable the server for streaming over the network");
...
DEFINE_string(camera_resolution,		"640x360",		"Size of the camera frames to ask for."); // "1280x720" original
...
struct UserDatum : public op::Datum
{
    ...
    UserDatum(const bool boolServerOn_ = true) :
        boolServerOn{boolServerOn_}
    {}
};
    ...
    void workConsumer(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr)
    {
        try
        {
            ...
            if (datumsPtr != nullptr && !datumsPtr->empty())
            {
                ...
                const char key = (char)cv::waitKey(5000);
                if (key == 27)
                    this->stop();
            }
        }
...
// cwc this class is from 3_user_sync
// This worker will just read and return all the jpg files in a directory
...
int openPoseTutorialWrapper2()
{
    ...
    op::log("Starting thread(s) with wrapper main", op::Priority::High);
